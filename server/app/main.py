        ...
        for port in (80, 8000, 8080, 8081, 6667):
            for path in ("/api/data", "/json"):
                try:
                    ...
                    chunks = []
                    while True:
                        part = await asyncio.wait_for(reader.read(4096), timeout=2.5)
                        if not part:
                            break
                        chunks.append(part)
                    writer.close()
...
async def telemetry_upload(request: Request):
    ...
    try:
        ...
        # v7.46: REST API 上报后触发报警状态迁移（与 MQTT/vitals/ingest 一致）
        record_alarm_transition(device_id, level, reason, temp_f, hum_f, pres_f)
    except Exception as e:
