...
| v7.2 | 2026-09-14 | 修复 Docker 配置 6 个问题 |
| v7.3 | 2026-09-14 | 修复设备绑定显示 + 解绑功能 + mosquitto passwd 逻辑 |
| v7.4 | 2026-09-14 | 添加数据源配置界面，支持 6 类外部系统数据交互 |
| v7.20 | 2026-09-14 | 稳定版发布：多患者监护 + AI 评估 + 企微推送 + OTA 升级 |
| v7.21 | 2026-09-14 | 测试工程师 bug 修复 5 项 |
| v7.22 | 2026-09-14 | 体征趋势/用药时间线面板移至监护页底部 |
| v7.23 | 2026-09-14 | 实时监护体征数据不显示 — 时间窗口回退查询 |
| v7.24 | 2026-09-14 | 新增 /api/vitals 端点兼容 ESP8266 固件上报 |
| v7.25 | 2026-09-14 | 修复实时监护界面 HTML 结构 — div 嵌套错误 |
| v7.26 | 2026-09-14 | 体征数据入库去重 bug + 趋势图时间范围筛选 |
| v7.27 | 2026-09-14 | 刷新按钮图标改文字 + 设备卡片按钮不换行 |
| v7.28 | 2026-09-14 | 体征报警 INSERT 列值不匹配 + 前端报警加载错误静默吞没 |
| v7.29 | 2026-09-14 | 实时监护 WebSocket 不处理 vital 消息 + 无定时刷新 |
| v7.30 | 2026-09-14 | 实时监护各数据源独立加载 + 中文标签 + pr_hr 回填 ecg_hr |
| v7.31 | 2026-09-14 | 监护界面逐指标独立取值 + 数据源映射修复 |
| v7.32 | 2026-09-14 | 医嘱/IO/检验接口容错 + 前端不再弹错误窗 |
| v7.33 | 2026-09-14 | 监护记录自动创建 + 设备自动绑定 + 遥测回退查所有设备 |
| v7.34 | 2026-09-14 | 趋势图回退查遥测数据 — vitals 表空时用 telemetry 画曲线 |
| v7.35 | 2026-09-14 | HTTP 遥测端点 + 局域网扫描发现设备 |
| v7.36 | 2026-09-14 | 设备管理按钮文字横排不换行 |
| v7.37 | 2026-09-14 | 系统设置分页签重构 + 数据源独立保存 + 数据库直连字段 |
| v7.38 | 2026-09-15 | AI 模型 API Key 保存后重新打开显示空白 |
| v7.39 | 2026-09-15 | 监护记录为空 — REST API 上报体征后未自动创建监护记录 |
| v7.40 | 2026-09-15 | 报警无记录 — REST API 上报数据后未触发报警检查 |
| v7.41 | 2026-09-15 | 患者切换监护设备后数据查看不完整 |
| v7.42 | 2026-09-15 | 新增就诊记录表(patient_encounters)，支持多次住院和设备切换区分 |
| v7.43 | 2026-09-15 | 非管理员用户可见 AI 模型等标签页但保存报 403 权限错误 |
| v7.44 | 2026-09-15 | admin 登录失败 — 启动时自动校验并重置密码 |
| v7.45 | 2026-09-15 | 数据管理界面 — 备份导出/导入恢复/删除，数据库每天备份保留 2 天 |
| v7.46 | 2026-09-16 | 19 个 Bug 修复（P0 连接池/安全鉴权/P1 等，详见 RELEASE_v7.46.md） |
| v7.47 | 2026-09-17 | 大屏免登录投屏 + 体征趋势图重构（一指标一小图/缺口分段/30分~24时档位/空窗回退显示最近记录）+ 用药时间线同款时间档位 + 出入量按钮拆分入蓝出红 + 面板按钮风格统一 + 最近录入分页历史 + 发提醒最近5条/历史分页 |
| v7.48 | 2026-09-19 | 设备管理卡片加宽至440px，操作按钮（含编辑）单行显示（详见 RELEASE_v7.48.md） |
| v7.49 | 2026-09-19 | 体征趋势窗口内无数据时回退显示最近记录曲线，修复实时监护24H空白（详见 RELEASE_v7.49.md） |
