...
public:
    ...
    // 应用服务器下发的提醒消息（医生通过企微发送）
    // JSON: {"text":"..."}
    void applyReminderPayload(const String &json);
    // 获取最近一次提醒消息（屏幕显示，主循环读取后清空）
    String takeReminderText();
    // 是否有新的提醒消息待显示
    bool hasReminder() const { return _reminderPending; }
...
private:
    ...
    uint32_t _reminderTime = 0;   // 消息到达时间，用于自动过期
};
