...
static char _topicRem[64];   // 提醒消息（医生通过企微下发，屏幕显示）
...
void MqttMgr::begin() {
    ...
    snprintf(_topicRem,  sizeof(_topicRem),  "envmon/%s/reminder",   g_cfg.device_id);
    ...
    _client.setCallback([](const char *topic, const uint8_t *payload, size_t len) {
        ...
        } else if (strcmp(topic, _topicRem) == 0) {
            g_mqtt.applyReminderPayload(json);
        }
    });
}
...
bool MqttMgr::doConnect() {
    ...
    if (ok) {
        ...
        _client.subscribe(_topicRem, 1);   // 订阅提醒消息（医生通过企微下发）
...
void MqttMgr::applyReminderPayload(const String &json) {
    if (json.length() < 2 || json.indexOf('{') < 0) {
        Serial.println(F("[MQTT] Bad reminder payload"));
        return;
    }
    String text;
    if (!jsonGetString(json, "text", text)) {
        Serial.println(F("[MQTT] Reminder: no text field"));
        return;
    }
    _reminderText = text;
    _reminderPending = true;
    _reminderTime = millis();
    Serial.printf("[MQTT] Reminder received: %s\n", text.c_str());
}

String MqttMgr::takeReminderText() {
    if (!_reminderPending) return String();
