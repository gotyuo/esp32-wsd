...
void NetManager::handleDataJson() {
    ...
    j += "\"device_id\":\"" + String(g_cfg.device_id) + "\",";
    j += "\"fw\":\"" FW_VERSION "\",";
    j += "\"ssid\":\"" + String(getCurSsidForJson()) + "\",";
    j += "\"ip\":\"" + String(WiFi.localIP().toString()) + "\",";
    j += "\"rssi\":" + String(WiFi.RSSI()) + ",";
    j += "\"uptime\":" + String((unsigned)(millis() / 1000)) + ",";
    j += "\"alarm\":" + String((int)g_alarm.level()) + ",";
    j += "\"valid\":" + String(!isnan(g_last.temp_c) ? "true" : "false") + ",";
    ...
    j += "\"temp_c\":" + floatOrNull(g_last.temp_c, 1) + ",";
    j += "\"hum_pct\":" + floatOrNull(g_last.hum_pct, 1) + ",";
...
void NetManager::startDataService() {
    ...
    web.on("/json", HTTP_GET, [this]() { handleDataJson(); });
    ...
    Serial.println(F("[NET] Data routes: /data /json /history /api/data /api/history"));
}
